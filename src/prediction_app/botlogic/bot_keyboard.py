...
class StartKeyboard:
    ...
    def build(start_message_id: int, user_id: int) -> InlineKeyboardMarkup:
        callback_sweet = SweetPrediction(start_message_id=start_message_id, user_id=user_id)
        callback_strict = StrictPrediction(start_message_id=start_message_id, user_id=user_id)
        button1 = InlineKeyboardButton(text="Ласковое", callback_data=callback_sweet.pack())
        button2 = InlineKeyboardButton(text="Суровое", callback_data=callback_strict.pack())
        logger.info(
