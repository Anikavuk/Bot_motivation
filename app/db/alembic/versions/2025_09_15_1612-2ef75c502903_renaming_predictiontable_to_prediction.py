...
down_revision: Union[str, Sequence[str], None] = '84019eb733d6'
...
def downgrade() -> None:
    ...
    sa.Column('extended_prediction', sa.TEXT(), autoincrement=False, nullable=False),
    sa.ForeignKeyConstraint(['user_id'], ['user.id'], name=op.f('prediction_table_user_id_fkey'), ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name=op.f('prediction_table_pkey')),
    sa.UniqueConstraint('id', name=op.f('prediction_table_id_key'), postgresql_include=[], postgresql_nulls_not_distinct=False)
    )
